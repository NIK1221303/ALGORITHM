...
public class HeapSort implements Sort {
    ...
    public void sort() {
        ...
        for (int setIndex = 0; setIndex < data.getSets().size(); setIndex++) {
            ...
            heapify(data.getSet(setIndex).getData(), n);// make initial heap

            // System.out.println("HEAPIFY: "+data.getSet(0).getData()); // after initial
            // heap

            pop(data.getSet(setIndex).getData(), n);
    ...
    public void heapify(ArrayList<Integer> data, int heapsize) {
        ...
        int greater_child; // this store value of greater child (compare left and right, will compare this
                           // variable with parent value later)
        ...
        for (int i = last_child; i > 1; i--) { // 10,8,6,4,2
            ...
            if (!(i % 2 == 0) && data.get(i) < data.get(parent)) { // !(i%2==0) indicate it have only 1 child (left), if
                                                                   // left < parent skip current iteration
                continue;
            }
            ...
            if (i % 2 == 0) {
                ...
                if (data.get(i) < data.get(i - 1)) {// compare right child with left child
                    ...
                    // System.out.println("greater child: "+greater_child+ " smaller
                    // child:"+data.get(i));
            ...
            if (greater_child > data.get(parent)) { // swap parent with greater children
                ...
                // after swap need to check if the parent got swapped still have greater child
                // or not, call heapify() again
                heapify(data, heapsize);

            }

            // if(i%2==0){
            // System.out.println("["+data.get(parent)+","+data.get(i-1)+","+data.get(i)+"]");
            // // print new parent, left and right child

            // }else{
            // System.out.println("["+data.get(parent)+","+data.get(i)+"]"); // print new
    ...
    public void pop(ArrayList<Integer> data1, int n) { // largest at root , put into last index, minus 1 heapsize, call
                                                       // heapify
